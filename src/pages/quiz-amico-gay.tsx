...
const FALLBACK: QuizFrociarolaContent = { questions: [], results: [] };

export default function QuizAmicoGay() {
    ...
    return (
      <Layout>
        <div className="max-w-2xl mx-auto py-16 text-center">
          <NeonCard>
            <h1 className="text-3xl font-display text-accent neon-glow mb-3">
              QUIZ IN ARRIVO ✨
            </h1>
    ...
    <Layout>
      <div className="max-w-2xl mx-auto py-8">
        <div className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-display text-accent neon-glow mb-2">
            CHE AMICO GAY SEI?
          </h1>
          <p className="text-xl text-white/80">
            Il test definitivo per te e le tue bestie! 👯‍♀️
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {!showResult ? (
            ...
            >
              <NeonCard className="border-accent/50">
                <div className="mb-8">
                ...
                <div className="flex flex-col gap-4">
                  {safeQ.options.map((opt, i) => (
                    <button
                      ...
                      className="p-6 rounded-xl bg-white/5 border-2 border-white/10 hover:border-accent hover:bg-accent/20 transition-all text-left text-lg font-bold hover:scale-[1.02]"
                    >
                      {opt.text}
                    </button>
                  ))}
            ...
            >
              <NeonCard className="text-center py-12 border-accent/50">
                <div className="text-8xl mb-6 animate-float">{winner.icon}</div>
                <p className="text-2xl font-bold text-white/70 mb-2">
                  Nel tuo gruppo sei...
                </p>
                <h2 className="text-4xl md:text-5xl font-display text-accent neon-glow mb-6">
                  {winner.title}
                </h2>
                ...
                <ShareCard
                  title={`Sono ${winner.title} 👯‍♀️`}
                  text={`Ho fatto il quiz "Che amico gay sei?" su Miss Frociarola Italia e sono ${winner.title}! 💅 Provalo anche tu:`}
                />
              </NeonCard>
            </motion.div>
          )}
